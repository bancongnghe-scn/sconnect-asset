<div class="tw-max-h-dvh overflow-y-scroll">
    <table id="example2" class="table table-bordered table-hover dataTable dtr-inline"
        ...
        <template x-for="(organization, index) in data.organizations" :key="index">
            ...
            <!-- Khi total_price khác 0 -->
            <template x-for="(assetRegister, stt) in organization.asset_register" :key="stt">
                <tr x-show="organization.total_price !== 0">
                    <td x-text="index + 1" :rowspan="stt === 0 ? organization.asset_register.length : null"
                        x-show="stt === 0"></td>
                    <td x-text="organization.name"
                        :rowspan="stt === 0 ? organization.asset_register.length : null"
                        x-show="stt === 0" class="tw-font-bold"></td>
                    ...
                    <td x-text="organization.total_price"
                        :rowspan="stt === 0 ? organization.asset_register.length : null"
                        x-show="stt === 0" class="text-center"></td>
                    <td :rowspan="stt === 0 ? organization.asset_register.length : null" x-show="stt === 0"
                        class="text-center">...</td>
                </tr>
            </template>

            <!-- Khi total_price là 0 -->
            <tr x-show="organization.total_price === 0">
                <td x-text="index + 1" rowspan="1"></td>
                <td x-text="organization.name" rowspan="1" class="tw-font-bold"></td>
                <template x-for="number in Array.from({ length: 14 }, (_, i) => i + 1)" :key="number">
