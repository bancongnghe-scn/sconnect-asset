                ...
                <table id="" class="table table-bordered dataTable dtr-inline"
                    ...
                    <tr :class="'position-sticky tw-top-0'">
                        <th class="text-center">
                            <input type="checkbox" id="selectedAllAssetDamaged" @click="selectedAllAssetDamaged"
                        ...
                        <tr>
                            ...
                            <template x-for="(columnName, key) in columns">
                                <td :class="'align-content-center'">
                                    <template
                                        x-if="key !== 'validity' && key !== 'status' && key !== 'date' && key !== 'user_name' ">
                                    ...
                                    <template x-if="key === 'user_name'">
                                        <div class="d-flex">
                                            ...
                                            <div
                                                ...
                                                <span x-text="data.user_name ?? ''"
                                                      style="font-weight: 600; font-size: 16px;"></span>
                                                <span x-text="data.user_code ? 'Mã nhân sự:' + data.user_code : ''"
                            ...
                            <td class="text-center align-middle">
                                <button class="border-0 position-relative" style="background-color: unset;"
                                        x-show="showAction.repaid ?? true"
                                        @click="$dispatch('repaid', { id: data.id })">
                                    <img src="/images/icon-setting.jpg" style="scale: 0.85">
                                    <span class="tooltip-text">Sửa chữa</span>
                                </button>
                                <button class="border-0 position-relative" style="background-color: unset;"
                                        x-show="showAction.liquidation ?? true"
                                        @click="$dispatch('liquidation', { id: data.id })">
                                    <img src="/images/icon-liquidation.jpg" style="scale: 0.85">
                                    <span class="tooltip-text">Đề nghị thanh lý</span>
                                </button>
                                <button class="border-0 position-relative" style="background-color: unset;"
                                        x-show="showAction.cancel ?? true"
                                        @click="$dispatch('cancel', { id: data.id })">
